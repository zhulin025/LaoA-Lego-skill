...
# LaoA-Lego skill：老 A 的3D 乐高模型生成器

## 简介
输入一个关键词或参考图片，生成详细的3D 乐高模型，同时生成一个可供直接打开并互动的网页。

## Skill 介绍
本 Skill 用于生成一个高质量的3D模型，使用乐高风格的积木构建。该模型应根据给定的关键词或参考图片进行设计。模型中应包含详细的微结构元素，比如面部特征、图案、像素化设计以及精细的绘画细节。此外，还提供交互式的网页界面，用户可以通过该界面来组装、拆卸模型，从不同角度查看模型，并放大或缩小模型中的各个部分。这个工具可以用来创建角色、机器人、交通工具、生物、建筑物等物体。它也非常适用于复制精确的图像，以及验证、修复、转换 BrickMorph JSON数据。
...
- 生成的模型网页会在右下角显示对比缩略图；有用户参考图时优先显示用户原图，否则显示建模概念图。
- 缩略图使用固定宽度和原图比例高度，完整显示图片，不再裁切。
- 点击缩略图可放大查看原图，并支持关闭按钮、点击背景或 `Esc` 键缩回。
- 旋转、缩放和平移操作说明移动到右上角明暗主题按钮下方。
